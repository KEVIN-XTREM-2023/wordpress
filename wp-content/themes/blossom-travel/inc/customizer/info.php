...
function blossom_travel_customizer_theme_info( $wp_customize ) {
    ...
    $theme_info = '<p>';
	$theme_info .= sprintf( __( 'Demo Link: %1$sClick here.%2$s', 'blossom-travel' ),  '<a href="' . esc_url( 'https://blossomthemes.com/theme-demo/?theme=blossom-travel/' ) . '" target="_blank">', '</a>' );
    $theme_info .= '</p><p>';
    $theme_info .= sprintf( __( 'Documentation Link: %1$sClick here.%2$s', 'blossom-travel' ),  '<a href="' . esc_url( 'https://docs.blossomthemes.com/blossom-travel/' ) . '" target="_blank">', '</a>' );
    $theme_info .= '</p>';
	$theme_info .= '<p>';
	...
	$manager->add_section(
		new Blossom_Travel_Customize_Section_Pro(
			...
			array(
				...
				'pro_url'  => esc_url( 'https://blossomthemes.com/wordpress-themes/blossom-travel-pro/?utm_source=free_theme&utm_medium=customizer&utm_campaign=pro_theme_upgrade' ),
			)
		)
	);
}
